...
Every cache exposes `clear()`, which removes all of its entries — useful when the current data
stops being valid for the session, such as on logout:
...
The base `clear()` routes through `delete()`, so a subclass that overrides `delete` to keep its
own bookkeeping sees every removal without also overriding `clear`. `LocalStorageCache` is the
exception: it drops its whole payload in a single write, so a subclass of *that* has to override
`clear` as well.
...
module-level cache reaches every test in the run. Reset it between tests with `clear()`, or give
the call a fresh instance each time.
